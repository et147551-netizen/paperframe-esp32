...
// src/epd_epdopt.c against byte for byte. This is what replaces judging a colour pipeline
...
//
// `--auto` writes test/test_auto/auto_fixtures.h, which pins src/epd_auto.c against
...
//     metrics exist. src/epd_auto.c's arm for it is reached by the `default:` label instead.
...
// src/epd_epdopt.c**, so the two-level chain the other two stages use (float vs double here,
...
// what src/epd_auto.c produces for them.
...
// the auto flow and have no double-precision counterpart in src/epd_epdopt.c: paper
...
// \`quality: "fast"\`, which is the algorithm src/epd_adjust.c implements -- upstream's auto flow
...
// src/epd_adjust.c is single precision. The bounds are in test_main.c and were written down
